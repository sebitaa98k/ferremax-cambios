...
def HistorialComprasView(request):
    # Ordena por fecha descendente y, si hay empate, por ID descendente
    ventas = Venta_productos.objects.filter(id_usuario=request.user, estado_venta='pagado').order_by('-fecha_transaccion', '-id')
